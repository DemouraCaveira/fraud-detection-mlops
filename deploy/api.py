...
# Eventos reais de /predict, para o painel "ao vivo" do visualizador do ciclo de vida
# (frontend/) via SSE em /events/live — lista simples em memoria (sem fila/lock: cada
# predicao roda no thread pool do FastAPI, e list.append e atomico o suficiente para o
# uso aqui; SSE so le, nunca remove enquanto adiciona).
_live_events: list[dict] = []
_LIVE_EVENTS_MAX = 500
...
async def lifespan(app: FastAPI):
    ...
    _state["reports_dir"] = reports_dir
...
# Origem do Vite dev server do visualizador (frontend/) — unico ajuste de
# infraestrutura para permitir que ele consuma esta API de outra porta em localhost.
app.add_middleware(
    CORSMiddleware,
...
# Serve os graficos reais ja gerados pelo pipeline (reports/figures/*.png) — usados
# pelo visualizador do ciclo de vida (frontend/) para mostrar o grafico de cada etapa,
# sem duplicar nenhuma logica de plot.
_figures_dir = resolve_path(load_config()["paths"]["reports_dir"]) / "figures"
...
def predict(transaction: Transaction):
    ...
    # publica um evento compacto para quem estiver ouvindo /events/live (o modo "ao
    # vivo" do visualizador do ciclo de vida) — a mesma predicao real que acabou de
    # rodar, sem recalcular nada
    _live_events.append({
        "type": "prediction", "fraud_probability": proba, "decision": decision,
        "latency_ms": latency_ms, "ts": time.time(),
    })
    if len(_live_events) > _LIVE_EVENTS_MAX:
        del _live_events[: len(_live_events) - _LIVE_EVENTS_MAX]
...
def _serialize_tree_node(node: dict, feature_names: list, x_row: dict) -> dict:
    """Converte um no do dump real do LightGBM (booster_.dump_model()) numa forma
    compacta para o frontend desenhar, marcando `on_path=True` no ramo que a transacao
    `x_row` de fato percorre — nenhum split/threshold/valor de folha e inventado, vem
    direto da arvore treinada."""
    if "left_child" not in node:
        return {"leaf": True, "value": float(node["leaf_value"]), "on_path": False}

    feature = feature_names[node["split_feature"]]
    threshold = float(node["threshold"])
    go_left = float(x_row[feature]) <= threshold

    left = _serialize_tree_node(node["left_child"], feature_names, x_row)
    right = _serialize_tree_node(node["right_child"], feature_names, x_row)
    (left if go_left else right)["on_path"] = True

    return {"leaf": False, "feature": feature, "threshold": threshold, "left": left, "right": right, "on_path": False}


def _leaf_value_on_path(node: dict) -> float:
    current = node
    while not current["leaf"]:
        current = current["left"] if current["left"]["on_path"] else current["right"]
    return current["value"]


@app.post("/model-internals")
def model_internals(transaction: Transaction):
    """Estrutura REAL das 202 arvores do modelo treinado (models/lightgbm_fraud.joblib),
    com o caminho real que ESTA transacao percorre em cada uma marcado — a mesma
    inferencia de /predict, so que mostrando o que acontece por dentro das arvores em
    vez de so o resultado final."""
    config = _state["config"]
    raw = transaction.model_dump()
    row_df = pd.DataFrame([raw])
    row_df = engineer_features(row_df, config)
    ...
    feature_cols = get_feature_columns(row_df, config)
    X = row_df[feature_cols]
    x_row = X.iloc[0].to_dict()

    booster = _state["model"].booster_
    feature_names = booster.feature_name()
    tree_info = booster.dump_model()["tree_info"]
    raw_score = float(booster.predict(X.values, raw_score=True)[0])

    trees = []
    leaf_sum = 0.0
    for t in tree_info:
        structure = _serialize_tree_node(t["tree_structure"], feature_names, x_row)
        structure["on_path"] = True  # a raiz e sempre percorrida
        leaf_value = _leaf_value_on_path(structure)
        leaf_sum += leaf_value
        trees.append({"index": t["tree_index"], "structure": structure, "leaf_value": leaf_value})

    return {
        "trees": trees,
        "n_trees": len(trees),
        "leaf_sum": leaf_sum,
        "init_score": raw_score - leaf_sum,
        "raw_score": raw_score,
        "fraud_probability": float(1.0 / (1.0 + np.exp(-raw_score))),
    }


def _read_json_report(filename: str) -> dict:
    """Wrapper fino: le e devolve um JSON ja gerado por um dos scripts do pipeline
    (src/evaluate.py, src/explainability.py, monitoring/drift_monitor.py) — nao
    recalcula nada, so serve o que ja existe em reports/."""
    path = _state["reports_dir"] / filename
    if not path.exists():
        raise HTTPException(
            status_code=404,
            detail=f"{filename} nao encontrado — rode o script correspondente do pipeline antes.",
        )
...
def drift_report():
    """PSI/KS reais (treino vs. teste + simulacao), gerados por monitoring/drift_monitor.py."""
    return _read_json_report("drift_report.json")


@app.get("/evaluation-summary")
def evaluation_summary_route():
    """Metricas, CV e benchmarks reais, gerados por src/evaluate.py."""
    return _read_json_report("evaluation_summary.json")


@app.get("/explainability-summary")
def explainability_summary_route():
    """Top atributos por SHAP, gerados por src/explainability.py."""
    return _read_json_report("explainability_summary.json")
...
async def events_live(request: Request):
    """SSE de predicoes reais: cada POST /predict que acontecer enquanto este stream
    estiver aberto e retransmitido aqui — usado pelo modo 'ao vivo' do visualizador do
    ciclo de vida (frontend/) para pulsar o no de Deploy/API com dado real."""

    async def gen():
        last_idx = len(_live_events)
        yield _sse_pack({"type": "connected", "message": "monitor ao vivo conectado"})
        while True:
            if await request.is_disconnected():
                break
            if len(_live_events) > last_idx:
                for event in _live_events[last_idx:]:
...
@app.get("/events/train-replay")
async def events_train_replay(speed: float = 1.0):
    """SSE que reproduz uma execucao real e ja capturada da pipeline (ver
    deploy/capture_training_events.py) respeitando os intervalos reais entre etapas,
    divididos por `speed` — nunca inventa duracao nem numero, so reproduz mais rapido."""
    path = _state["reports_dir"] / "training_events.json"
    if not path.exists():
        raise HTTPException(
            status_code=404,
            detail="reports/training_events.json nao encontrado — rode "
                   "`python deploy/capture_training_events.py` antes.",
        )
    with open(path, "r", encoding="utf-8") as f:
        events = json.load(f)
    speed = max(0.25, min(speed, 20.0))

    async def gen():
        prev_ts = 0.0
        for event in events:
            delay = (event["ts_offset_s"] - prev_ts) / speed
            prev_ts = event["ts_offset_s"]
            if delay > 0:
                await asyncio.sleep(delay)
            yield _sse_pack(event)
        yield _sse_pack({"type": "replay_finished"})
